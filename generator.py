...
def _pdf_styles():
    ...
    title_style = ParagraphStyle(
        "title", fontName="DejaVuSans-Bold", fontSize=14, alignment=TA_CENTER, leading=17,
    )
    ...
    cell_style = ParagraphStyle(
        "cell", fontName="DejaVuSans", fontSize=14, alignment=TA_LEFT, leading=17,
    )
    cell_bold_style = ParagraphStyle(
        "cell_bold", fontName="DejaVuSans-Bold", fontSize=14, alignment=TA_LEFT, leading=17,
    )
    header_style = ParagraphStyle(
        "header", fontName="DejaVuSans-Bold", fontSize=14, alignment=TA_CENTER, leading=17,
    )
...
def generate_combined_docx(lang_key: str, data: dict) -> str:
    ...
    def fill_cell(cell, label_key, value):
        cell.paragraphs[0].text = ""
        p1 = cell.paragraphs[0]
        run1 = p1.add_run(L[label_key]); run1.bold = True; run1.font.size = Pt(14)
        p2 = cell.add_paragraph(str(value))
        if p2.runs: p2.runs[0].font.size = Pt(14)
    for i, row_pairs in enumerate(rows):
        left, right = row_pairs
    ...
    r = heading.add_run(L["work_history_heading"]); r.bold = True; r.font.size = Pt(14)
    doc.add_paragraph()
    work_table = doc.add_table(rows=len(data["work_history"]), cols=3)
...
def generate_combined_pdf(lang_key: str, data: dict) -> str:
    ...
    photo_stream = _photo_3x4_stream(data)
    photo_flowable = RLImage(photo_stream, width=3 * rl_cm, height=4 * rl_cm) if photo_stream else Spacer(3 * rl_cm, 4 * rl_cm)
    name_style_header = ParagraphStyle("name_header", parent=name_style, fontSize=14, leading=17, alignment=TA_CENTER)
