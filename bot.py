...
async def menu_balance(message: Message):
    try:
        ...
        logging.exception("DB xatosi: get_user_balance")
        balance = 0
    await message.answer(f"💰 Balansingiz: {balance:,} soʻm".replace(",", " "))
...
async def send_admin_stats(message: Message):
    try:
        ...
        documents = await db.get_document_count()
        income = await db.get_total_income()
    except Exception:
        logging.exception("DB xatosi: admin stat")
    ...
    await message.answer(
        ...
        f"📄 Yaratilgan fayllar: {documents}\n"
        f"💰 Tasdiqlangan tushum: {income:,} soʻm".replace(",", " ")
    )
...
async def cmd_admin(message: Message, state: FSMContext):
    if not is_admin(message):
        ...
        return
    await state.clear()
    await message.answer("🔐 Administrator paneli:", reply_markup=admin_panel_kb())


@dp.callback_query(F.data == "admin:stats")
async def admin_stats_callback(callback: CallbackQuery):
    if not callback.from_user or callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Ruxsat yoʻq.", show_alert=True)
        return
    await callback.answer()
...
async def admin_broadcast_callback(callback: CallbackQuery, state: FSMContext):
    if not callback.from_user or callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Ruxsat yoʻq.", show_alert=True)
        return
    await state.set_state(AdminBroadcastForm.waiting_message)
    await callback.answer()
...
# MUHIM: /cancel handleri broadcast FSM handleridan OLDIN turishi kerak.
# Aks holda /cancel matni broadcast xabari sifatida barcha foydalanuvchilarga
# yuborilib ketadi.
@dp.message(Command("cancel"))
async def cmd_cancel_admin_or_any_state(message: Message, state: FSMContext):
    current_state = await state.get_state()
    await state.clear()

    if current_state == AdminBroadcastForm.waiting_message:
        await message.answer("❌ Broadcast bekor qilindi. Foydalanuvchilarga hech narsa yuborilmadi.")
    else:
        # Bu yerda say() ishlatilmaydi, chunki state clear qilingandan keyin
        # til/skript maʼlumoti yoʻqolishi mumkin.
        await message.answer("❌ Jarayon bekor qilindi. Qayta boshlash uchun /start bosing.")


@dp.message(AdminBroadcastForm.waiting_message)
async def process_broadcast(message: Message, state: FSMContext):
    if not is_admin(message):
        await state.clear()
        return

    # /cancel alohida handlerda ushlanadi; bu handlerga faqat haqiqiy
    # broadcast xabari keladi.
    try:
        user_ids = await db.get_all_user_ids()
    except Exception:
        logging.exception("DB xatosi: get_all_user_ids")
        await message.answer("❌ Foydalanuvchilar roʻyxatini olishda xatolik.")
        await state.clear()
        return
    ...
    for chat_id in user_ids:
        try:
            ...
            failed += 1
            logging.warning("Broadcast %s ga yuborilmadi: %s", chat_id, exc)
